...
resource "null_resource" "install_python_dependencies" {
  provisioner "local-exec" {
    command = "/bin/bash ${path.module}/scripts/create_pkg.sh"
...
resource "aws_lambda_function" "aws_lambda_test" {
  function_name = var.function_name
  description   = "Print Http Headers for the request"
  handler       = "lambda_function.PrintHttpHeaders-tf.lambda_handler"
  runtime       = var.runtime
